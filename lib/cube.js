...
class Cube {
  ...
  async put (location, content) {
    const params = { Bucket: this.bucket, Key: location, Body: content }
   ...
   */
  async get (location) {
    const res = await this.driver.getObject({ Bucket: this.bucket, Key: location })
   ...
   */
  async copy (from, to) {
    const src = `/${this.bucket}/${from}`

    const res = await this.driver.copyObject({ Bucket: this.bucket, Key: to, CopySource: src })
   ...
   */
  async meta (location) {
    const res = await this.driver.headObject({ Bucket: this.bucket, Key: location })
  ...
  async delete (location) {
    const res = await this.driver.deleteObject({ Bucket: this.bucket, Key: location })
   ...
   */
  path (location) {
    return [this.url, this.bucket, location].join('/')
  }
}
